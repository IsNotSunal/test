---@diagnostic disable: lowercase-global, redundant-parameter, param-type-mismatch
...
local TargetHUD = {
  ...
  on_render_screen = function(ctx, partial_ticks, width, height, scale_factor, mouse_x, mouse_y)
    world.entities()
    health = world.health(EnemyTarget())
    target = targetting()
    -- target check
    if not target and module_manager.is_module_on("killaura") and client.gui_name() == "chat" or client.gui_name() == "astolfo" then
      target = player.id()
      health = world.health(EnemyTarget())
    ...
    -- targethud misc
    local entity_name = world.name(EnemyTarget())
    ...
    if health ~= nil then
      if module_manager.option("TargetHUDs", "Default HUD") then
        ...
        local calc_bar_width = bar_width * health_relative(EnemyTarget())
        local health_bar_color = health_color(EnemyTarget())
        local hurt_time_width = width * (world.hurt_time(EnemyTarget()) / 9)
        ...
        if health >= 4 then
          render.player_head(optionX + math.ceil(animated_health_bar) - 29.5, optionY + height -31, 30, EnemyTarget())
        end
        if health >= 10 then
      ...
      elseif module_manager.option("TargetHUDs", "Novoline HUD") then
        ...
        -- new target check
        if EnemyTarget() ~= new_target then
          new_target = EnemyTarget()
          animated_health_bar = calc_bar_width
        end
        ...
        -- render the entity head
        render.player_head(optionX + 1, optionY + 1, 37, EnemyTarget())
        -- render health text
      ...
      elseif module_manager.option("TargetHUDs", "Novoline New HUD") then
        ...
        -- render the entity head
        render.player_head(optionX + 1, optionY + 1, 34.5, EnemyTarget())
        -- render health text
      ...
      elseif module_manager.option("TargetHUDs", "Moon HUD") then
        ...
        render.player(optionX + 18, optionY + 30, 14, 0, 0, EnemyTarget())
        -- render health text
        render.scale(0.91)
      ...
      elseif module_manager.option("TargetHUDs",'Astolfo Old HUD') then
        ...
        -- render the entity player
        render.player(optionX + 12, optionY + 43, 19, 0, 0, EnemyTarget())
         --render health text
      ...
      elseif module_manager.option("TargetHUDs", 'Astolfo Large HUD') then
        ...
        -- render the entity player
        render.player(optionX + 21, optionY + 56, 27, -25, 0, EnemyTarget())
         --render health text
        render.scale(2)
        scale = 2
      ...
      elseif module_manager.option("TargetHUDs", 'Exhibition HUD') then
        ...
        --render entity
        render.player(optionX + 18.5, optionY + 33.5, 16, 0, 0, EnemyTarget())
        --render health text and distance to entity
        render.scale(.6)
        render.string_shadow("HP: " ..round(health /2, 1) .. " \226\157\152 Dist: ".. math.floor(player.distance_to_entity(EnemyTarget())), optionX/.6 + 65, optionY/.6 + 30, 255, 255, 255, 255)
        render.scale(1/.6)
      elseif module_manager.option('TargetHUDs', 'Flux HUD') then
        ...
        if world.health(EnemyTarget()) ~= nil then
          if not has_value(targetList, EnemyTarget()) then
            table.insert(targetList, EnemyTarget())
          end
          if EnemyTarget() == nil or module_manager.is_module_on("killaura") and client.gui_name() == "chat" or client.gui_name() == "astolfo" then
            if not has_value(targetList, EnemyTarget()) then
      ...
      elseif module_manager.option('TargetHUDs', 'PowerX HUD') then
        ...
        render.scale(1/1)
        -- render player head
        render.player_head(optionX + 1.5, optionY + 1, 26, EnemyTarget())
      elseif module_manager.option('TargetHUDs', 'Woman HUD') then
        ...
        -- render head
        render.player_head(optionX + .5, optionY + .5, 25, EnemyTarget())
        -- render heart
      ...
      elseif module_manager.option('TargetHUDs', 'Rise HUD') then
        ...
        render.scale(0.9)
        render.string("Distance " .. round(player.distance_to_entity(EnemyTarget()),1) .. " Hurt " .. math.floor(world.hurt_time(EnemyTarget())), optionX/0.9 + 40, optionY/0.9+ 26, 255, 255, 255, 255)
        render.scale(1/0.9)
        -- render the health bar
        ...
        -- render the entity head
        hurt_time = math.max(7, world.hurt_time(EnemyTarget()))
        animated_hurt_time = exponential_anim(animated_hurt_time, hurt_time, 1)
        render.player_head(hurt_time + optionX - 5, hurt_time + optionY - 2, 44 - hurt_time * 2, EnemyTarget())
