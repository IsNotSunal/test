...
local renderHelper = {
    renderOutline = function(minX, minY, minZ, maxX, maxY, maxZ, sc, red, green, blue, alpha, lineWidth)
        ...
        local x1, y1, z1 = render.world_to_screen(dMaxX,dMinY,dMinZ)
        local x2, y2, z2 = render.world_to_screen(dMaxX,dMaxY,dMinZ)
        local x3, y3, z3 = render.world_to_screen(dMinX,dMinY,dMaxZ)
		local x4, y4, z4 = render.world_to_screen(dMinX,dMaxY,dMaxZ)
        local x5, y5, z5 = render.world_to_screen(dMinX,dMinY,dMinZ)
        local x6, y6, z6 = render.world_to_screen(dMinX,dMaxY,dMinZ)
        local x7, y7, z7 = render.world_to_screen(dMaxX,dMinY,dMaxZ)
        local x8, y8, z8 = render.world_to_screen(dMaxX,dMaxY,dMaxZ)
...
local function inRange(x, y, z, range)
...
local function checkForBed(x, y, z)
	local block = world.block(x, y, z)

	return block ~= nil and block == "tile.bed"
end

local function findSecondBedPart(coords)
	local offsets = {
		{ x = -1, z = 0 },
		{ x = 1, z = 0 },
		{ x = 0, z = 1 },
...
local function searchForDefense(bed_cover, bed_defense_block)
...
local function findBedCover(bed_blocks)
	local bed_cover = {}
...
local function findNearestBed(radius)
...
local function findHotbarSlot(item)
	for i = 1, 9 do
...
local function renderBlockOutline(event, x, y, z)
...
local block_breaker = {
	...
	on_pre_update = function()
		...
		if block ~= nil then
			...
			if slot ~= -1 then
				local smartcheck = player.inventory.slot(35 + slot)
